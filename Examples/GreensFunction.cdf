(* Content-type: application/vnd.wolfram.cdf.text *)

(*** Wolfram CDF File ***)
(* http://www.wolfram.com/cdf *)

(* CreatedBy='Mathematica 8.0' *)

(*************************************************************************)
(*                                                                       *)
(*  The Mathematica License under which this file was created prohibits  *)
(*  restricting third parties in receipt of this file from republishing  *)
(*  or redistributing it by any means, including but not limited to      *)
(*  rights management or terms of use, without the express consent of    *)
(*  Wolfram Research, Inc.                                               *)
(*                                                                       *)
(*************************************************************************)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[       835,         17]
NotebookDataLength[    194542,       3864]
NotebookOptionsPosition[    167500,       3276]
NotebookOutlinePosition[    193754,       3837]
CellTagsIndexPosition[    193641,       3831]
WindowTitle->Green's Function
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 ButtonBox[
  GraphicsBox[RasterBox[CompressedData["
1:
tu9+yuhi+5YoUaJEiZ559FT2rUSJEm2eLnZtSJQoUaJEiST6/wFtJUc4
    "], {{0, 0}, {1714, 38}}, {0, 255},
    ColorFunction->RGBColor],
   ImageSize->{1714, 38},
   PlotRange->{{0, 1714}, {0, 38}}],
  Alignment->Left,
  BaseStyle->{"Hyperlink", "DemonstrationHeader"},
  ButtonData->{
    URL["http://demonstrations.wolfram.com"], None},
  ButtonNote->"http://demonstrations.wolfram.com"]], "DemonstrationHeader"],

Cell["Green's Function", "DemoTitle"],

Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`bc$$ = $CellContext`periodic, \
$CellContext`m$$ = 0.1, $CellContext`mm$$ = Positive, $CellContext`n$$ = 1, 
    Typeset`show$$ = True, Typeset`bookmarkList$$ = {}, 
    Typeset`bookmarkMode$$ = "Menu", Typeset`animator$$, Typeset`animvar$$ = 
    1, Typeset`name$$ = "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`n$$], 1, "order"}, {
      1 -> "  1  ", 2 -> "  2  ", 3 -> "  3  ", 4 -> "  4  "}}, {{
       Hold[$CellContext`mm$$], Positive, "sign"}, {
      Positive -> "positive", Negative -> "negative"}}, {
      Hold[
       Grid[{{
          Manipulate`Place[1], "    ", 
          Manipulate`Place[2]}}]], Manipulate`Dump`ThisIsNotAControl}, {
      Hold[$CellContext`m$$], 0.1, 10, 0.001}, {{
       Hold[$CellContext`bc$$], $CellContext`periodic, "boundary conditions"}, 
      Dynamic[
       Which[$CellContext`n$$ == 
        1, {$CellContext`periodic -> " periodic ", $CellContext`bc2 -> 
         " initial ", $CellContext`bc3 -> " final "}, $CellContext`n$$ == 
        2, {$CellContext`periodic -> " periodic ", $CellContext`bc2 -> 
         " Dirichlet ", $CellContext`bc3 -> " Neumann "}, $CellContext`n$$ == 
        3, {$CellContext`periodic -> " periodic "}, $CellContext`n$$ == 
        4, {$CellContext`periodic -> " periodic ", $CellContext`bc2 -> 
         " simply supported beam ", $CellContext`bc3 -> " clamped beam "}]]}},
     Typeset`size$$ = {550., {173., 177.}}, Typeset`update$$ = 0, 
    Typeset`initDone$$, Typeset`skipInitDone$$ = 
    True, $CellContext`n$19143$$ = False, $CellContext`mm$19144$$ = 
    False, $CellContext`m$19145$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`bc$$ = $CellContext`periodic, \
$CellContext`m$$ = 0.1, $CellContext`mm$$ = Positive, $CellContext`n$$ = 1}, 
      "ControllerVariables" :> {
        Hold[$CellContext`n$$, $CellContext`n$19143$$, False], 
        Hold[$CellContext`mm$$, $CellContext`mm$19144$$, False], 
        Hold[$CellContext`m$$, $CellContext`m$19145$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Quiet[
        Module[{$CellContext`a$, $CellContext`b$}, $CellContext`a$ = 
          0; $CellContext`b$ = 1; Which[
           
           And[$CellContext`n$$ === 1, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`periodic], \
$CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{-(
                E^($CellContext`m ($CellContext`b + $CellContext`s - \
$CellContext`t))/(E^($CellContext`a $CellContext`m) - 
                E^($CellContext`b $CellContext`m))), $CellContext`a <= \
$CellContext`s <= $CellContext`t <= $CellContext`b}, {
              E^($CellContext`m ($CellContext`a + $CellContext`s - \
$CellContext`t))/(-E^($CellContext`a $CellContext`m) + 
               E^($CellContext`b $CellContext`m)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 1, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`bc2], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{
              E^($CellContext`m ($CellContext`s - $CellContext`t)), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}}, 0], 
           
           And[$CellContext`n$$ === 1, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ == $CellContext`bc3], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{
              0, $CellContext`a <= $CellContext`s <= $CellContext`t <= \
$CellContext`b}, {-E^($CellContext`m ($CellContext`s - $CellContext`t)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 1, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`periodic], \
$CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{
              E^($CellContext`m ($CellContext`a - $CellContext`s + \
$CellContext`t))/(E^($CellContext`a $CellContext`m) - 
               E^($CellContext`b $CellContext`m)), $CellContext`a <= \
$CellContext`s <= $CellContext`t <= $CellContext`b}, {
              E^($CellContext`m ($CellContext`b - $CellContext`s + \
$CellContext`t))/(E^($CellContext`a $CellContext`m) - 
               E^($CellContext`b $CellContext`m)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 1, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`bc2], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{
              E^($CellContext`m (-$CellContext`s + $CellContext`t)), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}}, 0], 
           
           And[$CellContext`n$$ === 1, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`bc3], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{
              0, $CellContext`a <= $CellContext`s <= $CellContext`t <= \
$CellContext`b}, {-E^($CellContext`m (-$CellContext`s + $CellContext`t)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 2, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`periodic], \
$CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{-((1/(
                 2 $CellContext`m)) ((
                   Cos[((1/
                    2) $CellContext`m) ($CellContext`a - $CellContext`b + 
                    2 $CellContext`s)] Cos[$CellContext`m $CellContext`t]) 
                  Csc[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m] + (
                   Csc[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m] 
                   Sin[((1/
                    2) $CellContext`m) ($CellContext`a - $CellContext`b + 
                    2 $CellContext`s)]) Sin[$CellContext`m $CellContext`t] - 
                 2 Sin[$CellContext`m (-$CellContext`s + $CellContext`t)])), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}, {-(
                Cos[((1/2) $CellContext`m) ($CellContext`a - $CellContext`b + 
                   2 $CellContext`s - 2 $CellContext`t)] (
                 Csc[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m]/(
                 2 $CellContext`m))), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 2, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`bc2], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(
                1/$CellContext`m) ((((-Cos[$CellContext`m $CellContext`t]) 
                   Csc[($CellContext`a - $CellContext`b) $CellContext`m]) 
                  Sin[$CellContext`a $CellContext`m]) 
                 Sin[$CellContext`m ($CellContext`b - $CellContext`s)] + ((
                   Cos[$CellContext`a $CellContext`m] 
                   Csc[($CellContext`a - $CellContext`b) $CellContext`m]) 
                  Sin[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                 Sin[$CellContext`m $CellContext`t] + 
                Sin[$CellContext`m (-$CellContext`s + $CellContext`t)]), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}, {-((
                 Csc[($CellContext`a - $CellContext`b) $CellContext`m] 
                 Sin[$CellContext`m ($CellContext`b - $CellContext`s)]) (
                 Sin[$CellContext`m ($CellContext`a - \
$CellContext`t)]/$CellContext`m)), 
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 2, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`bc3], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(
                1/$CellContext`m) ((((-Cos[$CellContext`a $CellContext`m]) 
                   Cos[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                  Cos[$CellContext`m $CellContext`t]) 
                 Csc[($CellContext`a - $CellContext`b) $CellContext`m] - ((
                  Cos[$CellContext`m ($CellContext`b - $CellContext`s)] 
                  Csc[($CellContext`a - $CellContext`b) $CellContext`m]) 
                 Sin[$CellContext`a $CellContext`m]) 
                Sin[$CellContext`m $CellContext`t] + 
                Sin[$CellContext`m (-$CellContext`s + $CellContext`t)]), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}, {-((
                 Cos[$CellContext`m ($CellContext`b - $CellContext`s)] 
                 Cos[$CellContext`m ($CellContext`a - $CellContext`t)]) (
                 Csc[($CellContext`a - $CellContext`b) \
$CellContext`m]/$CellContext`m)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 2, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`periodic], \
$CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(-E^($CellContext`m ($CellContext`s - $CellContext`t)) + 
                E^($CellContext`m (-$CellContext`s + $CellContext`t)) + 
                Cosh[((1/2) $CellContext`m) ($CellContext`a - $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)] 
                 Csch[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m])/(
               2 $CellContext`m), $CellContext`a <= $CellContext`s <= \
$CellContext`t <= $CellContext`b}, {
              Cosh[((1/2) $CellContext`m) ($CellContext`a - $CellContext`b + 
                  2 $CellContext`s - 2 $CellContext`t)] (
                Csch[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m]/(
                2 $CellContext`m)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 2, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`bc2], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(1/(
                2 $CellContext`m)) (-
                 E^($CellContext`m ($CellContext`s - $CellContext`t)) + 
                E^($CellContext`m (-$CellContext`s + $CellContext`t)) - ((2 
                  Csch[($CellContext`a - $CellContext`b) $CellContext`m]) 
                 Sinh[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                Sinh[$CellContext`m ($CellContext`a - $CellContext`t)]), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}, {-((
                 Csch[($CellContext`a - $CellContext`b) $CellContext`m] 
                 Sinh[$CellContext`m ($CellContext`b - $CellContext`s)]) (
                 Sinh[$CellContext`m ($CellContext`a - \
$CellContext`t)]/$CellContext`m)), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           And[$CellContext`n$$ === 2, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`bc3], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(1/(
                2 $CellContext`m)) (-
                 E^($CellContext`m ($CellContext`s - $CellContext`t)) + 
                E^($CellContext`m (-$CellContext`s + $CellContext`t)) + (((2 
                    Cosh[$CellContext`a $CellContext`m]) 
                   Cosh[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                  Cosh[$CellContext`m $CellContext`t]) 
                 Csch[($CellContext`a - $CellContext`b) $CellContext`m] - (((
                   2 Cosh[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                  Csch[($CellContext`a - $CellContext`b) $CellContext`m]) 
                 Sinh[$CellContext`a $CellContext`m]) 
                Sinh[$CellContext`m $CellContext`t]), $CellContext`a <= \
$CellContext`s <= $CellContext`t <= $CellContext`b}, {(
                Cosh[$CellContext`m ($CellContext`b - $CellContext`s)] 
                Cosh[$CellContext`m ($CellContext`a - $CellContext`t)]) (
                Csch[($CellContext`a - $CellContext`b) \
$CellContext`m]/$CellContext`m), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 3, $CellContext`mm$$ === 
            Positive], $CellContext`bc$$ = $CellContext`periodic; \
$CellContext`G[
              Pattern[$CellContext`t, 
               Blank[]], 
              Pattern[$CellContext`s, 
               Blank[]], 
              Pattern[$CellContext`a, 
               Blank[]], 
              Pattern[$CellContext`b, 
               Blank[]], 
              Pattern[$CellContext`m, 
               Blank[]]] := 
            Piecewise[{{-((
                  1/(((3 (E^($CellContext`a $CellContext`m) - 
                    E^($CellContext`b $CellContext`m))) $CellContext`m^2) (
                   E^($CellContext`a $CellContext`m) + 
                   E^($CellContext`b $CellContext`m) - (2 
                    E^(((1/2) ($CellContext`a + $CellContext`b)) \
$CellContext`m)) Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) \
$CellContext`m]))) (
                  E^((-$CellContext`m) ($CellContext`a - $CellContext`s + \
$CellContext`t)) (E^((2 $CellContext`a + $CellContext`b) $CellContext`m) + 
                   E^(($CellContext`a + 2 $CellContext`b) $CellContext`m) - (
                    2 E^(((3/
                    2) ($CellContext`a + $CellContext`b)) $CellContext`m)) 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) $CellContext`m] + \
((E^($CellContext`m (2 $CellContext`a - $CellContext`s + $CellContext`t)))^(3/
                    2) - (E^($CellContext`m (4 $CellContext`a + 
                    2 $CellContext`b - 3 $CellContext`s + 3 $CellContext`t)))^
                    Rational[1, 2]) 
                    Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
(E^($CellContext`m ($CellContext`a + $CellContext`b - $CellContext`s + \
$CellContext`t)))^(3/2) 
                    Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)] - (
                    E^($CellContext`m (5 $CellContext`a + $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2] 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)] + (
                    3^Rational[1, 2] (
                    E^($CellContext`m (
                    2 $CellContext`a - $CellContext`s + $CellContext`t)))^(3/
                    2)) Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] - \
(3^Rational[1, 2] (E^($CellContext`m (4 $CellContext`a + 2 $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
(3^Rational[1, 
                    2] (E^($CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s + $CellContext`t)))^(3/2)) 
                    Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)] - (
                    3^Rational[1, 2] (
                    E^($CellContext`m (5 $CellContext`a + $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)]))), $CellContext`a <= $CellContext`s <= \
$CellContext`t <= $CellContext`b}, {-((
                  1/(((3 (E^($CellContext`a $CellContext`m) - 
                    E^($CellContext`b $CellContext`m))) $CellContext`m^2) (
                   E^($CellContext`a $CellContext`m) + 
                   E^($CellContext`b $CellContext`m) - (2 
                    E^(((1/2) ($CellContext`a + $CellContext`b)) \
$CellContext`m)) Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) \
$CellContext`m]))) (
                  E^((-$CellContext`m) ($CellContext`b - $CellContext`s + \
$CellContext`t)) (E^((2 $CellContext`a + $CellContext`b) $CellContext`m) + 
                   E^(($CellContext`a + 2 $CellContext`b) $CellContext`m) - (
                    2 E^(((3/
                    2) ($CellContext`a + $CellContext`b)) $CellContext`m)) 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) $CellContext`m] + \
(E^($CellContext`m (2 $CellContext`b - $CellContext`s + $CellContext`t)))^(3/
                    2) Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] - \
(E^($CellContext`m (2 $CellContext`a + 4 $CellContext`b - 3 $CellContext`s + 
                    3 $CellContext`t)))^Rational[1, 2] 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
((E^($CellContext`m ($CellContext`a + $CellContext`b - $CellContext`s + \
$CellContext`t)))^(3/2) - (
                    E^($CellContext`m ($CellContext`a + 5 $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                    Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)] + (
                    3^Rational[1, 2] (
                    E^($CellContext`m (
                    2 $CellContext`b - $CellContext`s + $CellContext`t)))^(3/
                    2)) 
                    Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] - \
(3^Rational[1, 2] (E^($CellContext`m (2 $CellContext`a + 4 $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
(3^Rational[1, 
                    2] (E^($CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s + $CellContext`t)))^(3/2)) 
                    Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)] - (
                    3^Rational[1, 2] (
                    E^($CellContext`m ($CellContext`a + 5 $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)]))), 
                
                Inequality[$CellContext`a, Less, $CellContext`t, 
                 Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 3, $CellContext`mm$$ === 
            Negative], $CellContext`bc$$ = $CellContext`periodic; \
$CellContext`G[
              Pattern[$CellContext`t, 
               Blank[]], 
              Pattern[$CellContext`s, 
               Blank[]], 
              Pattern[$CellContext`a, 
               Blank[]], 
              Pattern[$CellContext`b, 
               Blank[]], 
              Pattern[$CellContext`m, 
               Blank[]]] := 
            Piecewise[{{(
                 1/(((3 (E^($CellContext`a $CellContext`m) - 
                    E^($CellContext`b $CellContext`m))) $CellContext`m^2) (
                  E^($CellContext`a $CellContext`m) + 
                  E^($CellContext`b $CellContext`m) - (2 
                   E^(((1/2) ($CellContext`a + $CellContext`b)) \
$CellContext`m)) 
                  Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) \
$CellContext`m]))) ((
                  E^((-$CellContext`m) ($CellContext`a - $CellContext`s + \
$CellContext`t)) (
                    E^($CellContext`m ($CellContext`a - $CellContext`s + \
$CellContext`t)))^Rational[1, 2]) (
                  E^(((1/2) $CellContext`m) (5 $CellContext`a - 
                    3 $CellContext`s + 3 $CellContext`t)) + 
                  E^($CellContext`b $CellContext`m + ((3/
                    2) $CellContext`m) ($CellContext`a - $CellContext`s + \
$CellContext`t)) - (
                   2 (E^($CellContext`m (4 $CellContext`a + $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                  Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) $CellContext`m] + \
(E^($CellContext`a ($CellContext`m/2) + $CellContext`b $CellContext`m) (-
                    E^($CellContext`a $CellContext`m) + 
                    E^($CellContext`b $CellContext`m))) 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + 
                  E^(((1/2) (
                    4 $CellContext`a + $CellContext`b)) $CellContext`m) 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)] - 
                  E^($CellContext`a $CellContext`m + (
                    3 $CellContext`b) ($CellContext`m/2)) 
                  Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)] - (3^Rational[1, 2] 
                   E^(((1/2) ($CellContext`a + 
                    4 $CellContext`b)) $CellContext`m)) 
                  Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
(3^Rational[1, 2] E^((3 $CellContext`a) ($CellContext`m/
                    2) + $CellContext`b $CellContext`m)) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] - \
(3^Rational[1, 2] 
                   E^(((1/2) (
                    4 $CellContext`a + $CellContext`b)) $CellContext`m)) 
                  Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)] + (3^Rational[1, 2] 
                    E^($CellContext`a $CellContext`m + (
                    3 $CellContext`b) ($CellContext`m/2))) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) (-$CellContext`a + $CellContext`b + \
$CellContext`s - $CellContext`t)])), $CellContext`a <= $CellContext`s <= \
$CellContext`t <= $CellContext`b}, {(
                 1/(((3 (E^($CellContext`a $CellContext`m) - 
                    E^($CellContext`b $CellContext`m))) $CellContext`m^2) (
                  E^($CellContext`a $CellContext`m) + 
                  E^($CellContext`b $CellContext`m) - (2 
                   E^(((1/2) ($CellContext`a + $CellContext`b)) \
$CellContext`m)) 
                  Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) \
$CellContext`m]))) ((
                  E^((-$CellContext`m) ($CellContext`b - $CellContext`s + \
$CellContext`t)) (
                    E^($CellContext`m ($CellContext`b - $CellContext`s + \
$CellContext`t)))^Rational[1, 2]) (
                  E^(((1/2) $CellContext`m) (5 $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)) + 
                  E^($CellContext`a $CellContext`m + ((3/
                    2) $CellContext`m) ($CellContext`b - $CellContext`s + \
$CellContext`t)) - (
                   2 (E^($CellContext`m ($CellContext`a + 4 $CellContext`b - 
                    3 $CellContext`s + 3 $CellContext`t)))^Rational[1, 2]) 
                  Cos[(((1/2) 
                    3^Rational[
                    1, 2]) ($CellContext`a - $CellContext`b)) $CellContext`m] + 
                  E^(((1/2) (
                    4 $CellContext`a + $CellContext`b)) $CellContext`m) 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] - 
                  E^($CellContext`a $CellContext`m + (
                    3 $CellContext`b) ($CellContext`m/2)) 
                  Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
(E^($CellContext`a ($CellContext`m/2) + $CellContext`b $CellContext`m) (-
                    E^($CellContext`a $CellContext`m) + 
                    E^($CellContext`b $CellContext`m))) 
                   Cos[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)] - (3^Rational[1, 2] 
                   E^(((1/2) (
                    4 $CellContext`a + $CellContext`b)) $CellContext`m)) 
                  Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] + \
(3^Rational[1, 2] E^($CellContext`a $CellContext`m + (
                    3 $CellContext`b) ($CellContext`m/2))) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`s - $CellContext`t)] - \
(3^Rational[1, 2] 
                   E^(((1/2) ($CellContext`a + 
                    4 $CellContext`b)) $CellContext`m)) 
                  Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)] + (3^Rational[1, 2] 
                    E^((3 $CellContext`a) ($CellContext`m/
                    2) + $CellContext`b $CellContext`m)) 
                   Sin[(((1/2) 
                    3^Rational[
                    1, 2]) $CellContext`m) ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)])), 
                
                Inequality[$CellContext`a, Less, $CellContext`t, 
                 Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 4, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`periodic], \
$CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{((
                 Cosh[$CellContext`m ((3 $CellContext`a - 
                    3 $CellContext`b - $CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] - 
                 Sinh[$CellContext`m ((3 $CellContext`a - 
                    3 $CellContext`b - $CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]) (
                 Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/(2 
                    2^Rational[1, 2]))] (
                   Sinh[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])] (
                    Cosh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] (
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] + 
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) + (
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] - 
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))]) - 
                   
                   Cosh[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])] (
                    Cosh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] (
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] - 
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) + (
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])] + 
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))])) + 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/(2 
                    2^Rational[1, 2]))] ((-
                    Sinh[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) ((
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] + 
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) 
                    Cosh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] + (
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] - 
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))]) + 
                   Cosh[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])] (
                    Cosh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] (-
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])] + 
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) + (
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] + 
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b - \
$CellContext`s + $CellContext`t)/2^Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`a - $CellContext`b - 
                    2 $CellContext`s + 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))]))))/((((2 
                  2^Rational[1, 2]) $CellContext`m^3) (
                 Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])] - 
                 Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])])) (
                Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                Sinh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m])), $CellContext`a <= $CellContext`s <= $CellContext`t <= \
$CellContext`b}, {((
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/(2 
                    2^Rational[1, 2]))] (
                   Cosh[$CellContext`m (($CellContext`a - $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] ((-
                    Cosh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])]) (
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] + 
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])]) + (
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])] + 
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])]) + 
                   Sinh[$CellContext`m (($CellContext`a - $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] (
                    Cosh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])] (
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])] - 
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) + (-
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])] + 
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])])) + 
                 Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/(2 
                    2^Rational[1, 2]))] (
                   Sinh[$CellContext`m (($CellContext`a - $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] (
                    Cosh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])] (
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] - 
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])]) + (
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])] - 
                    Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])]) + 
                   Cosh[$CellContext`m (($CellContext`a - $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)/(2 
                    2^Rational[1, 2]))] (
                    Cosh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])] (
                    Sin[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])] + 
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) - (
                    Cos[$CellContext`m (($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)/2^Rational[1, 2])] + 
                    Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sinh[$CellContext`m (($CellContext`s - $CellContext`t)/2^
                    Rational[1, 2])]))) (
                 Cosh[$CellContext`m ((2 $CellContext`a - 
                    2 $CellContext`b - $CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] - 
                 Sinh[$CellContext`m ((2 $CellContext`a - 
                    2 $CellContext`b - $CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]))/((((2 
                  2^Rational[1, 2]) $CellContext`m^3) (
                 Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])] - 
                 Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])])) (
                Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                Sinh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m])), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 4, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`bc3], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{-((((((2^Rational[1, 2] 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                   Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                   Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m])))) - (2^Rational[1, 2] 
                Cosh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(
                    2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] 
                 Cosh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] 
                 Cosh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - ((((((2 2^Rational[1, 2]) 
                    Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((((2 2^Rational[1, 2]) 
                    Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((((2 2^Rational[1, 2]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((((2 2^Rational[1, 2]) 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2 2^Rational[1, 2]) 
                   Sin[$CellContext`m ((-$CellContext`a + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (2^Rational[1, 2] 
                 Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (
                Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] 
                Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(
                    2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (
                Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])] 
                Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]) (
                Sinh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))), $CellContext`a <= $CellContext`s <= $CellContext`t <= \
$CellContext`b}, {-((((((2^Rational[1, 2] 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                   Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                   Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m])))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((3 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`s)/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((
                  Cos[$CellContext`m (($CellContext`a - 
                    2 $CellContext`b + $CellContext`s)/2^Rational[1, 2])] 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((((2 2^Rational[1, 2]) 
                    Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(
                    2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((((2 2^Rational[1, 2]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + ((((((2 2^Rational[1, 2]) 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (-2 + 
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2 2^Rational[1, 2]) 
                   Sin[$CellContext`m ((-$CellContext`a + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (-2 + 
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] + 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 4, $CellContext`mm$$ === 
            Positive, $CellContext`bc$$ === $CellContext`bc2], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{-((((((2^Rational[1, 2] 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                   Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                   Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m])))) + (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] 
                 Cosh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (
                Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] 
                Cosh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sin[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]/($CellContext`m^3 (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]/($CellContext`m^3 (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (
                Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] 
                Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                   Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (
                 Cos[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])] 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]) (
                 Sinh[$CellContext`m ((-$CellContext`s + $CellContext`t)/2^
                    Rational[1, 2])]/((2^Rational[1, 2] $CellContext`m^3) (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))), $CellContext`a <= $CellContext`s <= $CellContext`t <= \
$CellContext`b}, {-((((((2^Rational[1, 2] 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                  Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                   Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                   Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m])))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]) (
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]/($CellContext`m^3 (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                   Rational[1, 2])]/($CellContext`m^3 (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                    Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) - (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Cosh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                 Sin[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                   Rational[1, 2])]) (
                Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                   Rational[1, 2])]/($CellContext`m^3 (
                 Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                 Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                   Cosh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                  Sin[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))) + (((((2^Rational[1, 2] 
                    Cos[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                    Cos[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) 
                   Cos[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]) 
                  Sinh[(-$CellContext`a + $CellContext`b) ($CellContext`m/2^
                    Rational[1, 2])]) 
                 Sinh[$CellContext`m ((-$CellContext`b + $CellContext`s)/2^
                    Rational[1, 2])]) (
                 Sinh[$CellContext`m ((-$CellContext`a + $CellContext`t)/2^
                    Rational[1, 2])]/($CellContext`m^3 (
                  Cos[(2^Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) $CellContext`m] - 
                  Cosh[(2^
                    Rational[
                    1, 2] (-$CellContext`a + $CellContext`b)) \
$CellContext`m]))), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 4, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`periodic], \
$CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(
                1/((4 (E^($CellContext`a $CellContext`m) - 
                  E^($CellContext`b $CellContext`m))) $CellContext`m^3)) (
                E^((-$CellContext`m) ($CellContext`a - $CellContext`s + \
$CellContext`t)) (E^(($CellContext`a + $CellContext`b) $CellContext`m) + 
                 E^((
                    2 $CellContext`m) ($CellContext`a - $CellContext`s + \
$CellContext`t)) + ((
                    E^($CellContext`m (
                    2 $CellContext`a - $CellContext`s + $CellContext`t)) - 
                    E^($CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s + $CellContext`t))) 
                   Cos[((1/
                    2) $CellContext`m) (-$CellContext`a + $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)]) 
                  Csc[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m])), \
$CellContext`a <= $CellContext`s <= $CellContext`t <= $CellContext`b}, {(
                1/((4 (E^($CellContext`a $CellContext`m) - 
                  E^($CellContext`b $CellContext`m))) $CellContext`m^3)) (
                E^((-$CellContext`m) ($CellContext`b - $CellContext`s + \
$CellContext`t)) (E^(($CellContext`a + $CellContext`b) $CellContext`m) + 
                 E^((2 $CellContext`m) ($CellContext`b - $CellContext`s + \
$CellContext`t)) + ((
                    E^($CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s + $CellContext`t)) - 
                    E^($CellContext`m (
                    2 $CellContext`b - $CellContext`s + $CellContext`t))) 
                   Cos[((1/
                    2) $CellContext`m) ($CellContext`a - $CellContext`b + 
                    2 $CellContext`s - 2 $CellContext`t)]) 
                  Csc[((1/
                    2) ($CellContext`a - $CellContext`b)) $CellContext`m])), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 4, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`bc3], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(
                 1/((4 $CellContext`m^3) (
                  2 E^(($CellContext`a + $CellContext`b) $CellContext`m) - (
                   E^((2 $CellContext`a) $CellContext`m) + 
                   E^((2 $CellContext`b) $CellContext`m)) 
                  Cos[($CellContext`a - $CellContext`b) $CellContext`m]))) (
                 E^((-$CellContext`m) ($CellContext`b + $CellContext`s + \
$CellContext`t)) (
                  E^($CellContext`m ($CellContext`a + 
                    2 ($CellContext`b + $CellContext`s))) - 
                  E^($CellContext`m ($CellContext`a + 
                    2 ($CellContext`b + $CellContext`t))) + (-
                    E^($CellContext`m (2 $CellContext`a + $CellContext`b + 
                    2 $CellContext`s)) + 
                    E^((3 $CellContext`b) $CellContext`m + (
                    2 $CellContext`m) $CellContext`t)) 
                   Cos[($CellContext`a - $CellContext`b) $CellContext`m] + (
                    E^($CellContext`m (2 $CellContext`b + $CellContext`s)) (
                    E^((2 $CellContext`a) $CellContext`m) - 
                    E^((2 $CellContext`m) $CellContext`t))) 
                   Cos[$CellContext`m ($CellContext`a - $CellContext`s)] - 
                  E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`s)) 
                  Cos[$CellContext`m ($CellContext`b - $CellContext`s)] + 
                  E^($CellContext`m ($CellContext`a + $CellContext`b + \
$CellContext`s + 2 $CellContext`t)) 
                   Cos[$CellContext`m ($CellContext`b - $CellContext`s)] + 
                  E^($CellContext`m (
                    2 ($CellContext`a + $CellContext`b) + $CellContext`t)) 
                   Cos[$CellContext`m ($CellContext`a - $CellContext`t)] - 
                  E^($CellContext`m (
                    2 ($CellContext`b + $CellContext`s) + $CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`a - $CellContext`t)] - 
                  E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`b - $CellContext`t)] + 
                  E^($CellContext`m ($CellContext`a + $CellContext`b + 
                    2 $CellContext`s + $CellContext`t)) 
                   Cos[$CellContext`m ($CellContext`b - $CellContext`t)] - 
                  E^($CellContext`m (
                    2 $CellContext`a + $CellContext`b + $CellContext`s + \
$CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s - $CellContext`t)] + 
                  E^($CellContext`m (
                    3 $CellContext`b + $CellContext`s + $CellContext`t)) 
                   Cos[$CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s - $CellContext`t)] + 
                  E^((2 $CellContext`a) $CellContext`m + (
                    3 $CellContext`b) $CellContext`m) 
                   Sin[($CellContext`a - $CellContext`b) $CellContext`m] + 
                  E^($CellContext`m ($CellContext`b + 
                    2 ($CellContext`s + $CellContext`t))) 
                   Sin[($CellContext`a - $CellContext`b) $CellContext`m] - 
                  E^($CellContext`m (2 $CellContext`a + 
                    2 $CellContext`b + $CellContext`s)) 
                  Sin[$CellContext`m ($CellContext`a - $CellContext`s)] - 
                  E^($CellContext`m (2 $CellContext`b + $CellContext`s + 
                    2 $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`a - $CellContext`s)] + 
                  E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`s)) 
                   Sin[$CellContext`m ($CellContext`b - $CellContext`s)] + 
                  E^($CellContext`m ($CellContext`a + $CellContext`b + \
$CellContext`s + 2 $CellContext`t)) 
                   Sin[$CellContext`m ($CellContext`b - $CellContext`s)] - 
                  E^($CellContext`m (
                    2 ($CellContext`a + $CellContext`b) + $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`a - $CellContext`t)] - 
                  E^($CellContext`m (
                    2 ($CellContext`b + $CellContext`s) + $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`a - $CellContext`t)] + 
                  E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`t)) 
                   Sin[$CellContext`m ($CellContext`b - $CellContext`t)] + 
                  E^($CellContext`m ($CellContext`a + $CellContext`b + 
                    2 $CellContext`s + $CellContext`t)) 
                   Sin[$CellContext`m ($CellContext`b - $CellContext`t)] - (2 
                   E^($CellContext`m ($CellContext`a + 
                    2 $CellContext`b + $CellContext`s + $CellContext`t))) 
                  Sin[$CellContext`m ($CellContext`s - $CellContext`t)] + 
                  E^($CellContext`m (
                    2 $CellContext`a + $CellContext`b + $CellContext`s + \
$CellContext`t)) 
                   Sin[$CellContext`m ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)] + 
                  E^($CellContext`m (
                    3 $CellContext`b + $CellContext`s + $CellContext`t)) 
                   Sin[$CellContext`m ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)])) - (
                E^($CellContext`m ($CellContext`s - $CellContext`t)) - 
                E^($CellContext`m (-$CellContext`s + $CellContext`t)) + 
                2 Sin[$CellContext`m (-$CellContext`s + $CellContext`t)])/(
               4 $CellContext`m^3), $CellContext`a <= $CellContext`s <= \
$CellContext`t <= $CellContext`b}, {(
                1/((4 $CellContext`m^3) (
                 2 E^(($CellContext`a + $CellContext`b) $CellContext`m) - (
                  E^((2 $CellContext`a) $CellContext`m) + 
                  E^((2 $CellContext`b) $CellContext`m)) 
                 Cos[($CellContext`a - $CellContext`b) $CellContext`m]))) (
                E^((-$CellContext`m) ($CellContext`b + $CellContext`s + \
$CellContext`t)) (
                 E^($CellContext`m ($CellContext`a + 
                    2 ($CellContext`b + $CellContext`s))) - 
                 E^($CellContext`m ($CellContext`a + 
                   2 ($CellContext`b + $CellContext`t))) + (-
                    E^($CellContext`m (2 $CellContext`a + $CellContext`b + 
                    2 $CellContext`s)) + 
                   E^((3 $CellContext`b) $CellContext`m + (
                    2 $CellContext`m) $CellContext`t)) 
                  Cos[($CellContext`a - $CellContext`b) $CellContext`m] + (
                   E^($CellContext`m (2 $CellContext`b + $CellContext`s)) (
                    E^((2 $CellContext`a) $CellContext`m) - 
                    E^((2 $CellContext`m) $CellContext`t))) 
                  Cos[$CellContext`m ($CellContext`a - $CellContext`s)] - 
                 E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`s)) 
                 Cos[$CellContext`m ($CellContext`b - $CellContext`s)] + 
                 E^($CellContext`m ($CellContext`a + $CellContext`b + \
$CellContext`s + 2 $CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`b - $CellContext`s)] + 
                 E^($CellContext`m (
                    2 ($CellContext`a + $CellContext`b) + $CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`a - $CellContext`t)] - 
                 E^($CellContext`m (
                    2 ($CellContext`b + $CellContext`s) + $CellContext`t)) 
                 Cos[$CellContext`m ($CellContext`a - $CellContext`t)] - 
                 E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`t)) 
                 Cos[$CellContext`m ($CellContext`b - $CellContext`t)] + 
                 E^($CellContext`m ($CellContext`a + $CellContext`b + 
                    2 $CellContext`s + $CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`b - $CellContext`t)] - 
                 E^($CellContext`m (
                    2 $CellContext`a + $CellContext`b + $CellContext`s + \
$CellContext`t)) 
                 Cos[$CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s - $CellContext`t)] + 
                 E^($CellContext`m (
                    3 $CellContext`b + $CellContext`s + $CellContext`t)) 
                  Cos[$CellContext`m ($CellContext`a + $CellContext`b - \
$CellContext`s - $CellContext`t)] + 
                 E^((2 $CellContext`a) $CellContext`m + (
                    3 $CellContext`b) $CellContext`m) 
                  Sin[($CellContext`a - $CellContext`b) $CellContext`m] + 
                 E^($CellContext`m ($CellContext`b + 
                    2 ($CellContext`s + $CellContext`t))) 
                  Sin[($CellContext`a - $CellContext`b) $CellContext`m] - 
                 E^($CellContext`m (2 $CellContext`a + 
                    2 $CellContext`b + $CellContext`s)) 
                 Sin[$CellContext`m ($CellContext`a - $CellContext`s)] - 
                 E^($CellContext`m (2 $CellContext`b + $CellContext`s + 
                    2 $CellContext`t)) 
                 Sin[$CellContext`m ($CellContext`a - $CellContext`s)] + 
                 E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`s)) 
                  Sin[$CellContext`m ($CellContext`b - $CellContext`s)] + 
                 E^($CellContext`m ($CellContext`a + $CellContext`b + \
$CellContext`s + 2 $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`b - $CellContext`s)] - 
                 E^($CellContext`m (
                    2 ($CellContext`a + $CellContext`b) + $CellContext`t)) 
                 Sin[$CellContext`m ($CellContext`a - $CellContext`t)] - 
                 E^($CellContext`m (
                    2 ($CellContext`b + $CellContext`s) + $CellContext`t)) 
                 Sin[$CellContext`m ($CellContext`a - $CellContext`t)] + 
                 E^($CellContext`m ($CellContext`a + 
                    3 $CellContext`b + $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`b - $CellContext`t)] + 
                 E^($CellContext`m ($CellContext`a + $CellContext`b + 
                    2 $CellContext`s + $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`b - $CellContext`t)] - (2 
                  E^($CellContext`m ($CellContext`a + 
                    2 $CellContext`b + $CellContext`s + $CellContext`t))) 
                 Sin[$CellContext`m ($CellContext`s - $CellContext`t)] + 
                 E^($CellContext`m (
                    2 $CellContext`a + $CellContext`b + $CellContext`s + \
$CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)] + 
                 E^($CellContext`m (
                    3 $CellContext`b + $CellContext`s + $CellContext`t)) 
                  Sin[$CellContext`m ($CellContext`a - $CellContext`b + \
$CellContext`s - $CellContext`t)])), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0], 
           
           And[$CellContext`n$$ === 4, $CellContext`mm$$ === 
            Negative, $CellContext`bc$$ === $CellContext`bc2], $CellContext`G[
             Pattern[$CellContext`t, 
              Blank[]], 
             Pattern[$CellContext`s, 
              Blank[]], 
             Pattern[$CellContext`a, 
              Blank[]], 
             Pattern[$CellContext`b, 
              Blank[]], 
             Pattern[$CellContext`m, 
              Blank[]]] := 
           Piecewise[{{(Sin[$CellContext`m ($CellContext`s - $CellContext`t)] - 
                 Sinh[$CellContext`m ($CellContext`s - $CellContext`t)])/(
                2 $CellContext`m^3) + (1/(
                 4 $CellContext`m^3)) (((
                   E^($CellContext`m ($CellContext`b - $CellContext`s)) 
                   Csc[($CellContext`a - $CellContext`b) $CellContext`m]) 
                  Csch[($CellContext`a - $CellContext`b) $CellContext`m]) \
((((-1 + E^((2 $CellContext`m) (-$CellContext`b + $CellContext`s))) 
                    Cosh[$CellContext`m $CellContext`t]) 
                    Sin[($CellContext`a - $CellContext`b) $CellContext`m]) 
                   Sinh[$CellContext`a $CellContext`m] + ((((2 
                    E^($CellContext`m (-$CellContext`b + $CellContext`s))) 
                    Cosh[$CellContext`b $CellContext`m]) 
                    Sin[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                    Sin[$CellContext`m ($CellContext`a - $CellContext`t)]) 
                   Sinh[$CellContext`a $CellContext`m] - 
                  Cosh[$CellContext`a $CellContext`m] ((((2 
                    E^($CellContext`m (-$CellContext`b + $CellContext`s))) 
                    Sin[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                    Sin[$CellContext`m ($CellContext`a - $CellContext`t)]) 
                    Sinh[$CellContext`b $CellContext`m] + ((-1 + 
                    E^((2 $CellContext`m) (-$CellContext`b + $CellContext`s)))
                     Sin[($CellContext`a - $CellContext`b) $CellContext`m]) 
                    Sinh[$CellContext`m $CellContext`t]))), $CellContext`a <= \
$CellContext`s <= $CellContext`t <= $CellContext`b}, {(1/(
                4 $CellContext`m^3)) (((
                  E^($CellContext`m ($CellContext`b - $CellContext`s)) 
                  Csc[($CellContext`a - $CellContext`b) $CellContext`m]) 
                 Csch[($CellContext`a - $CellContext`b) $CellContext`m]) \
((((-1 + E^((2 $CellContext`m) (-$CellContext`b + $CellContext`s))) 
                    Cosh[$CellContext`m $CellContext`t]) 
                   Sin[($CellContext`a - $CellContext`b) $CellContext`m]) 
                  Sinh[$CellContext`a $CellContext`m] + ((((2 
                    E^($CellContext`m (-$CellContext`b + $CellContext`s))) 
                    Cosh[$CellContext`b $CellContext`m]) 
                    Sin[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                   Sin[$CellContext`m ($CellContext`a - $CellContext`t)]) 
                  Sinh[$CellContext`a $CellContext`m] - 
                 Cosh[$CellContext`a $CellContext`m] ((((2 
                    E^($CellContext`m (-$CellContext`b + $CellContext`s))) 
                    Sin[$CellContext`m ($CellContext`b - $CellContext`s)]) 
                    Sin[$CellContext`m ($CellContext`a - $CellContext`t)]) 
                   Sinh[$CellContext`b $CellContext`m] + ((-1 + 
                    E^((2 $CellContext`m) (-$CellContext`b + $CellContext`s)))
                     Sin[($CellContext`a - $CellContext`b) $CellContext`m]) 
                   Sinh[$CellContext`m $CellContext`t]))), 
               
               Inequality[$CellContext`a, Less, $CellContext`t, 
                Less, $CellContext`s, LessEqual, $CellContext`b]}}, 0]]; 
         Plot3D[
           $CellContext`G[$CellContext`t, $CellContext`s, $CellContext`a$, \
$CellContext`b$, $CellContext`m$$], {$CellContext`s, $CellContext`a$, \
$CellContext`b$}, {$CellContext`t, $CellContext`a$, $CellContext`b$}, 
           PlotRange -> Full, ImageSize -> {550, 350}, SphericalRegion -> 
           True]]], 
      "Specifications" :> {{{$CellContext`n$$, 1, "order"}, {
         1 -> "  1  ", 2 -> "  2  ", 3 -> "  3  ", 4 -> "  4  "}, 
         ControlPlacement -> 1}, {{$CellContext`mm$$, Positive, "sign"}, {
         Positive -> "positive", Negative -> "negative"}, ControlType -> 
         RadioButton, ControlPlacement -> 2}, 
        Grid[{{
           Manipulate`Place[1], "    ", 
           Manipulate`Place[2]}}], {$CellContext`m$$, 0.1, 10, 0.001, 
         Appearance -> "Labeled", ImageSize -> 
         Small}, {{$CellContext`bc$$, $CellContext`periodic, 
          "boundary conditions"}, 
         Dynamic[
          
          Which[$CellContext`n$$ == 
           1, {$CellContext`periodic -> " periodic ", $CellContext`bc2 -> 
            " initial ", $CellContext`bc3 -> " final "}, $CellContext`n$$ == 
           2, {$CellContext`periodic -> " periodic ", $CellContext`bc2 -> 
            " Dirichlet ", $CellContext`bc3 -> " Neumann "}, $CellContext`n$$ == 
           3, {$CellContext`periodic -> " periodic "}, $CellContext`n$$ == 
           4, {$CellContext`periodic -> " periodic ", $CellContext`bc2 -> 
            " simply supported beam ", $CellContext`bc3 -> 
            " clamped beam "}]], ControlType -> SetterBar}}, 
      "Options" :> {
       SynchronousUpdating -> False, TrackedSymbols -> True, 
        AutorunSequencing -> {1}}, 
      "DefaultOptions" :> {ControllerLinking -> True}],
     ImageSizeCache->{595., {239., 244.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    SynchronousInitialization->True,
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output",
 CellID->983915261],

Cell[TextData[{
 "This Demonstration shows the graphic of the Green's function ",
 Cell[BoxData[
  FormBox[
   RowBox[{"G", "(", 
    RowBox[{"t", ",", "s"}], ")"}], TraditionalForm]], "InlineMath"],
 " related to the linear differential equation of order ",
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "1"}], TraditionalForm]], "InlineMath"],
 ", ",
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "2"}], TraditionalForm]], "InlineMath"],
 ", ",
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "3"}], TraditionalForm]], "InlineMath"],
 ", or ",
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "4"}], TraditionalForm]], "InlineMath"],
 ", "
}], "ManipulateCaption"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{
      SuperscriptBox["u", 
       RowBox[{"(", "n", ")"}]], "(", "s", ")"}], "+", " ", 
     RowBox[{"a", "  ", 
      SuperscriptBox["m", "n"], " ", 
      RowBox[{"u", "(", "s", ")"}]}]}], "=", 
    RowBox[{"\[Sigma]", "(", "s", ")"}]}], TraditionalForm]], "InlineMath"],
 ","
}], "ManipulateCaption",
 CellID->1379222796],

Cell[TextData[{
 "where ",
 Cell[BoxData[
  FormBox[
   RowBox[{"a", "=", 
    RowBox[{"\[PlusMinus]", "1"}]}], TraditionalForm]], "InlineMath"],
 " and with the following linear boundary conditions:"
}], "ManipulateCaption",
 CellID->659046654],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "1"}], TraditionalForm]], "InlineMath"],
 ": periodic: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "0", ")"}], "=", 
    RowBox[{"u", "(", "1", ")"}]}], TraditionalForm]], "InlineMath"],
 ", initial: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "0", ")"}], "=", "0"}], TraditionalForm]], "InlineMath"],
 ", or final: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "1", ")"}], "=", "0"}], TraditionalForm]], "InlineMath"],
 ";"
}], "ManipulateCaption",
 CellID->2023946199],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "2"}], TraditionalForm]], "InlineMath"],
 ": periodic: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"u", "(", "0", ")"}], "=", 
     RowBox[{"u", "(", "1", ")"}]}], ",", " ", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "0", ")"}]}], "=", 
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "1", ")"}]}]}]}], TraditionalForm]], "InlineMath"],
 ", Dirichlet: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "0", ")"}], "=", 
    RowBox[{
     RowBox[{"u", "(", "1", ")"}], "=", "0"}]}], TraditionalForm]], 
  "InlineMath"],
 " or Neumann: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"u", "'"}], 
     RowBox[{"(", "0", ")"}]}], "=", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "1", ")"}]}], "=", "0"}]}], TraditionalForm]], 
  "InlineMath"],
 ";"
}], "ManipulateCaption",
 CellID->1719721734],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "3"}], TraditionalForm]], "InlineMath"],
 ": periodic: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"u", "(", "0", ")"}], "=", 
     RowBox[{"u", "(", "1", ")"}]}], ",", " ", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "0", ")"}]}], "=", 
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "1", ")"}]}]}], ",", " ", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "''"}], 
      RowBox[{"(", "0", ")"}]}], "=", 
     RowBox[{
      RowBox[{"u", "''"}], 
      RowBox[{"(", "1", ")"}]}]}]}], TraditionalForm]], "InlineMath"],
 ";"
}], "ManipulateCaption",
 CellID->724192869],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{"n", "=", "4"}], TraditionalForm]], "InlineMath"],
 ": periodic: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"u", "(", "0", ")"}], "=", 
     RowBox[{"u", "(", "1", ")"}]}], ",", " ", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "0", ")"}]}], "=", 
     RowBox[{
      RowBox[{"u", "'"}], 
      RowBox[{"(", "1", ")"}]}]}], ",", " ", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "''"}], 
      RowBox[{"(", "0", ")"}]}], "=", 
     RowBox[{
      RowBox[{"u", "''"}], 
      RowBox[{"(", "1", ")"}]}]}], ",", 
    RowBox[{
     RowBox[{
      RowBox[{"u", "'''"}], 
      RowBox[{"(", "0", ")"}]}], "=", 
     RowBox[{
      RowBox[{"u", "'''"}], 
      RowBox[{"(", "1", ")"}]}]}]}], TraditionalForm]], "InlineMath"],
 ", simply supported beam: ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "0", ")"}], "=", 
    RowBox[{
     RowBox[{"u", "(", "1", ")"}], "=", 
     RowBox[{
      RowBox[{
       RowBox[{"u", "''"}], 
       RowBox[{"(", "0", ")"}]}], "=", 
      RowBox[{
       RowBox[{
        RowBox[{"u", "''"}], 
        RowBox[{"(", "1", ")"}]}], "=", "0"}]}]}]}], TraditionalForm]], 
  "InlineMath"],
 ", or clamped beam ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "0", ")"}], "=", 
    RowBox[{
     RowBox[{"u", "(", "1", ")"}], "=", 
     RowBox[{
      RowBox[{
       RowBox[{"u", "'"}], 
       RowBox[{"(", "0", ")"}]}], "=", 
      RowBox[{
       RowBox[{
        RowBox[{"u", "'"}], 
        RowBox[{"(", "1", ")"}]}], "=", "0"}]}]}]}], TraditionalForm]], 
  "InlineMath"],
 "."
}], "ManipulateCaption",
 CellID->380778365],

Cell[TextData[{
 "The solution of the boundary value problem (differential equation and \
boundary conditions) is given by ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"u", "(", "t", ")"}], "=", 
    RowBox[{
     SubsuperscriptBox["\[Integral]", "0", "1"], 
     RowBox[{
      RowBox[{"G", "(", 
       RowBox[{"t", ",", "s"}], ")"}], 
      RowBox[{"\[Sigma]", "(", "s", ")"}], "d", "\[InvisibleSpace]", 
      "s"}]}]}], TraditionalForm]], "InlineMath"],
 "."
}], "ManipulateCaption",
 CellID->1641698573],

Cell[TextData[{
 "In the code, the expression for the corresponding Green's function is given \
for an arbitrary interval ",
 Cell[BoxData[
  FormBox[
   RowBox[{"[", 
    RowBox[{"a", ",", "b"}], "]"}], TraditionalForm]], "InlineMath"],
 "."
}], "ManipulateCaption",
 CellID->251648375],

Cell["THINGS TO TRY", "ManipulateCaption",
 CellFrame->{{0, 0}, {1, 0}},
 CellFrameColor->RGBColor[0.87, 0.87, 0.87],
 FontFamily->"Helvetica",
 FontSize->12,
 FontWeight->"Bold",
 FontColor->RGBColor[0.597406, 0, 0.0527047],
 CellTags->"ControlSuggestions"],

Cell[TextData[{
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Resize Images",
     FontFamily->"Verdana"]]], True->Cell[TextData[StyleBox["Resize Images",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]]]}, Dynamic[
     CurrentValue["MouseOver"]]],
   "\"Click inside an image to reveal its orange resize frame.\\nDrag any of \
the orange resize handles to resize the image.\"",
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]]],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Rotate and Zoom in 3D",
     FontFamily->"Verdana"]]], True->Cell[TextData[StyleBox[
    "Rotate and Zoom in 3D",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]]]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{
    "\"Drag a 3D graphic to rotate it. Starting the drag near the center \
tumbles\\nthe graphic; starting near a corner turns it parallel to the plane \
of the screen.\\nHold down \"", 
     FrameBox[
     "Ctrl", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" (or \"", 
     FrameBox[
     "Cmd", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" on Mac) and drag up and down to zoom.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]]],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Slider Zoom",
     FontFamily->"Verdana"]]], True->Cell[TextData[StyleBox["Slider Zoom",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]]]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{"\"Hold down the \"", 
     FrameBox[
     "Alt", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], 
     "\" key while moving a slider to make fine adjustments in the slider \
value.\\nHold \"", 
     FrameBox[
     "Ctrl", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" and/or \"", 
     FrameBox[
     "Shift", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" at the same time as \"", 
     FrameBox[
     "Alt", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" to make ever finer adjustments.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]]],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Gamepad Controls",
     FontFamily->"Verdana"]]], True->Cell[TextData[StyleBox["Gamepad Controls",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]]]}, Dynamic[
     CurrentValue["MouseOver"]]],
   "\"Control this Demonstration with a gamepad or other\\nhuman interface \
device connected to your computer.\"",
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]]],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Automatic Animation",
     FontFamily->"Verdana"]]], True->Cell[TextData[StyleBox[
    "Automatic Animation",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]]]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{"\"Animate a slider in this Demonstration by clicking the\"", 
     AdjustmentBox[
      Cell[
       GraphicsData[
       "CompressedBitmap", 
        "eJzzTSzJSM1NLMlMTlRwL0osyMhMLlZwyy8CCjEzMjAwcIKwAgOI/R/IhBKc\n\
/4EAyGAG0f+nTZsGwgysIJIRKsWKLAXGIHFmEpUgLADxWUAkI24jZs+eTaEt\n\
IG+wQKRmzJgBlYf5lhEA30OqWA=="], "Graphics", ImageSize -> {9, 9}, ImageMargins -> 
       0, CellBaseline -> Baseline], BoxBaselineShift -> 0.1839080459770115, 
      BoxMargins -> {{0., 0.}, {-0.1839080459770115, 0.1839080459770115}}], 
     "\"button\\nnext to the slider, and then clicking the play button that \
appears.\\nAnimate all controls by selecting \"", 
     StyleBox["Autorun", FontWeight -> "Bold"], "\" from the\"", 
     AdjustmentBox[
      Cell[
       GraphicsData[
       "CompressedBitmap", 
        "eJyNULENwyAQfEySIlMwTVJlCGRFsosokeNtqBmDBagoaZjAI1C8/8GUUUC6\n\
57h7cQ8PvU7Pl17nUav7oj/TPH7V7b2QJAUAXBkKmCPRowxICy64bRvGGNF7\n\
X8CctGoDSN4xhIDGGDhzFXwUh3/ClBKrDQPmnGXtI6u0OOd+tZBVUqy1xSaH\n\
UqiK6pPe4XdEdAz6563tx/gejuORGMxJaz8mdpJn7hc="], "Graphics", 
       ImageSize -> {10, 10}, ImageMargins -> 0, CellBaseline -> Baseline], 
      BoxBaselineShift -> 0.1839080459770115, 
      BoxMargins -> {{0., 0.}, {-0.1839080459770115, 0.1839080459770115}}], 
     "\"menu.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]]]
}], "ManipulateCaption",
 CellMargins->{{Inherited, Inherited}, {0, 0}},
 Deployed->True,
 FontFamily->"Verdana",
 CellTags->"ControlSuggestions"],

Cell["DETAILS", "DetailsSection"],

Cell["Reference", "DetailNotes",
 CellID->1965504561],

Cell[TextData[{
 "[1] E. A. Coddington and N. Levinson,",
 StyleBox[" Theory of Ordinary Differential Equations",
  FontSlant->"Italic"],
 ", New York-Toronto-London: McGraw\[Dash]Hill, 1955."
}], "DetailNotes",
 CellID->1552175217],

Cell["RELATED LINKS", "RelatedLinksSection"],

Cell[TextData[{
 ButtonBox["Green's Function",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://mathworld.wolfram.com/GreensFunction.html"], None},
  ButtonNote->"http://mathworld.wolfram.com/GreensFunction.html"],
 " (",
 ButtonBox["Wolfram",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://mathworld.wolfram.com/"], None},
  ButtonNote->"http://mathworld.wolfram.com/"],
 " ",
 StyleBox[ButtonBox["MathWorld",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://mathworld.wolfram.com/"], None},
  ButtonNote->"http://mathworld.wolfram.com/"],
  FontSlant->"Italic"],
 ")"
}], "RelatedLinks",
 CellID->670378459],

Cell[BoxData[
 ButtonBox[
  PaneSelectorBox[{False->Cell[BoxData[
    GraphicsBox[RasterBox[CompressedData["
1:
      "], {{0, 0}, {194, 22}}, {0, 255},
      ColorFunction->RGBColor],
     ImageSize->{194, 22},
     PlotRange->{{0, 194}, {0, 22}}]]], True->Cell[BoxData[
    GraphicsBox[RasterBox[CompressedData["
1:
      "], {{0, 0}, {194, 22}}, {0, 255},
      ColorFunction->RGBColor],
     ImageSize->{194, 22},
     PlotRange->{{0, 194}, {0, 22}}]]]}, Dynamic[
    CurrentValue["MouseOver"]]],
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/versions/source.jsp?id=\
GreensFunction&version=0007"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/GreensFunction/GreensFunction-source.\
nb"]], "DemoSourceNotebookSection",
 CellFrame->{{0, 0}, {1, 1}},
 ShowCellBracket->False,
 CellMargins->{{48, 48}, {28, 28}},
 CellGroupingRules->{"SectionGrouping", 25},
 CellFrameMargins->{{48, 48}, {6, 8}},
 CellFrameColor->RGBColor[0.87, 0.87, 0.87]],

Cell["PERMANENT CITATION", "CitationSection"],

Cell[TextData[{
 "\[NonBreakingSpace]",
 ButtonBox["Alberto Cabada",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Alberto+Cabada"]\
, None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Alberto+Cabada"],
 ", ",
 ButtonBox["Jos\[EAcute] \[CapitalAAcute]ngel Cid",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Jose+Angel+Cid"]\
, None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Jose+Angel+Cid"],
 ", and ",
 ButtonBox["Beatriz M\[AAcute]quez-Villamar\[IAcute]n",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Beatriz+M%\
E1quez-Villamar%EDn"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Beatriz+M%E1quez-\
Villamar%EDn"]
}], "Author",
 FontColor->GrayLevel[0.6]],

Cell[TextData[{
 "\"",
 ButtonBox["Green's Function",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/GreensFunction/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/GreensFunction/"],
 "\"",
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 ButtonBox["http://demonstrations.wolfram.com/GreensFunction/",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/GreensFunction/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/GreensFunction/"],
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 ButtonBox["Wolfram Demonstrations Project",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 "Published: ",
 "October 3, 2011"
}], "Citations"],

Cell[TextData[{
 "\[Copyright] ",
 StyleBox[ButtonBox["Wolfram Demonstrations Project & Contributors",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
  FontColor->GrayLevel[0.6]],
 "\[ThickSpace]\[ThickSpace]\[ThickSpace]|\[ThickSpace]\[ThickSpace]\
\[ThickSpace]",
 StyleBox[ButtonBox["Terms of Use",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/termsofuse.html"], None},
  ButtonNote->"http://demonstrations.wolfram.com/termsofuse.html"],
  FontColor->GrayLevel[0.6]]
}], "Text",
 CellFrame->{{0, 0}, {0, 0.5}},
 CellMargins->{{48, 48}, {20, 50}},
 CellFrameColor->GrayLevel[0.45098],
 FontFamily->"Verdana",
 FontSize->9,
 FontColor->GrayLevel[0.6],
 CellTags->"Copyright"]
},
Editable->False,
Saveable->False,
ScreenStyleEnvironment->"Working",
CellInsertionPointCell->None,
CellGrouping->Manual,
WindowSize->{750, 650},
WindowMargins->{{0, Automatic}, {Automatic, 0}},
WindowElements->{
 "StatusArea", "MemoryMonitor", "MagnificationPopUp", "VerticalScrollBar", 
  "MenuBar"},
WindowTitle->"Green's Function",
DockedCells->{},
CellContext->Notebook,
FrontEndVersion->"8.0 for Microsoft Windows (32-bit) (February 23, 2011)",
StyleDefinitions->Notebook[{
   Cell[
    CellGroupData[{
      Cell[
      "Demonstration Styles", "Title", 
       CellChangeTimes -> {
        3.3509184553711*^9, {3.36928902713192*^9, 3.36928902738193*^9}, {
         3.3754479092466917`*^9, 3.3754479095123196`*^9}, {
         3.375558447161495*^9, 3.375558447395873*^9}, {3.37572892702972*^9, 
         3.375728927639103*^9}}], 
      Cell[
       StyleData[StyleDefinitions -> "Default.nb"]], 
      Cell[
       CellGroupData[{
         Cell[
         "Style Environment Names", "Section", 
          CellChangeTimes -> {{3.369277974278112*^9, 3.369277974396138*^9}}], 
         Cell[
          StyleData[All, "Working"], ShowCellBracket -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Notebook Options", "Section", 
          CellChangeTimes -> {{3.374865264950812*^9, 3.374865265419568*^9}}], 
         Cell[
         "  The options defined for the style below will be used at the \
Notebook level.  ", "Text"], 
         Cell[
          StyleData["Notebook"], Editable -> True, 
          PageHeaders -> {{None, None, None}, {None, None, None}}, 
          PageFooters -> {{None, None, None}, {None, None, None}}, 
          PageHeaderLines -> {False, False}, 
          PageFooterLines -> {False, False}, 
          PrintingOptions -> {
           "FacingPages" -> False, "FirstPageFooter" -> False, 
            "RestPagesFooter" -> False}, CellFrameLabelMargins -> 6, 
          DefaultNewInlineCellStyle -> "InlineMath", DefaultInlineFormatType -> 
          "DefaultTextInlineFormatType", ShowStringCharacters -> True, 
          CacheGraphics -> False, StyleMenuListing -> None, 
          DemonstrationSite`Private`CreateCellID -> True, 
          DemonstrationSite`Private`TrackCellChangeTimes -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Input/Output", "Section", 
          CellChangeTimes -> {{3.3756313297791014`*^9, 
           3.3756313299509783`*^9}}], 
         Cell[
         "The cells in this section define styles used for input and output \
to the kernel.  Be careful when modifying, renaming, or removing these \
styles, because the front end associates special meanings with these style \
names.    ", "Text"], 
         Cell[
          StyleData["Input"], CellMargins -> {{48, 4}, {6, 4}}], 
         Cell[
          StyleData["Output"], CellMargins -> {{48, 4}, {6, 4}}], 
         Cell[
          StyleData["DemonstrationHeader"], Deletable -> False, 
          CellFrame -> {{0, 0}, {0, 0}}, ShowCellBracket -> False, 
          CellMargins -> {{0, 0}, {30, 0}}, 
          CellGroupingRules -> {"SectionGrouping", 20}, 
          CellHorizontalScrolling -> True, 
          CellFrameMargins -> {{0, 0}, {0, 0}}, CellFrameColor -> 
          RGBColor[0, 0, 0], StyleMenuListing -> None, Background -> 
          RGBColor[0, 0, 0]], 
         Cell[
          StyleData["ShowSource"], CellFrame -> {{0, 0}, {0, 1}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
          CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
          RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
          "Helvetica", FontSize -> 10, FontWeight -> "Bold", FontSlant -> 
          "Plain", FontColor -> RGBColor[1, 0.42, 0]]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Basic Styles", "Section", 
          CellChangeTimes -> {{3.34971724802035*^9, 3.34971724966638*^9}, {
           3.35091840608065*^9, 3.35091840781999*^9}, {3.35091845122987*^9, 
           3.35091845356607*^9}, {3.35686681885432*^9, 3.35686681945788*^9}, {
           3.375657418186455*^9, 3.375657418452083*^9}}], 
         Cell[
          StyleData["Hyperlink"], StyleMenuListing -> None, FontColor -> 
          GrayLevel[0]], 
         Cell[
          StyleData["SiteLink"], StyleMenuListing -> None, 
          ButtonStyleMenuListing -> Automatic, FontColor -> 
          GrayLevel[0.45098], 
          ButtonBoxOptions -> {
           Active -> True, Appearance -> {Automatic, None}, 
            ButtonFunction :> (FrontEndExecute[{
               NotebookLocate[#2]}]& ), ButtonNote -> ButtonData}], 
         Cell[
          StyleData["Link"], FontColor -> GrayLevel[0.45098]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoNotes"], CellFrame -> True, 
             CellMargins -> {{0, 0}, {0, 0}}, 
             CellFrameMargins -> {{48, 48}, {4, 4}}, CellFrameColor -> 
             GrayLevel[0.99], StyleMenuListing -> None, FontFamily -> 
             "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.45098], 
             DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
            Cell[
             StyleData["DemoNotes", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, FontSize -> 9]}, Open]], 
         Cell[
          StyleData["SnapshotsSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, ShowGroupOpener -> True, 
          CellMargins -> {{48, 48}, {10, 30}}, 
          PrivateCellOptions -> {"DefaultCellGroupOpen" -> False}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], DefaultNewCellStyle -> 
          "SnapshotCaption", StyleMenuListing -> None, FontFamily -> 
          "Verdana", FontSize -> 12, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "SnapshotCaption", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["SnapshotOutput"], ShowCellBracket -> False, 
             CellMargins -> {{48, 10}, {5, 7}}, Evaluatable -> True, 
             CellGroupingRules -> "InputGrouping", PageBreakWithin -> False, 
             GroupPageBreakWithin -> False, DefaultFormatType -> 
             DefaultInputFormatType, ShowAutoStyles -> True, 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, 
             AutoItalicWords -> {}, LanguageCategory -> "Mathematica", 
             FormatType -> InputForm, NumberMarks -> True, 
             LinebreakAdjustments -> {0.85, 2, 10, 0, 1}, CounterIncrements -> 
             "Input", MenuSortingValue -> 1500, MenuCommandKey -> "9", 
             DemonstrationSite`Private`StripStyleOnPaste -> True], 
            Cell[
             StyleData["SnapshotOuput", "Printout"], 
             CellMargins -> {{39, 0}, {4, 6}}, 
             LinebreakAdjustments -> {0.85, 2, 10, 1, 1}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoTitle"], Deletable -> False, ShowCellBracket -> 
             False, CellMargins -> {{48, 48}, {22, 10}}, 
             CellGroupingRules -> {"SectionGrouping", 20}, StyleMenuListing -> 
             None, FontFamily -> "Verdana", FontSize -> 20, FontWeight -> 
             "Bold", FontColor -> RGBColor[0.597406, 0, 0.0527047], 
             Background -> GrayLevel[1]], 
            Cell[
             StyleData["DemoName", "Printout"], 
             CellMargins -> {{24, 8}, {8, 27}}, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, FontSize -> 
             16]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DetailsSection"], CellFrame -> {{0, 0}, {1, 0}}, 
             ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
             CellGroupingRules -> {"SectionGrouping", 25}, 
             CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
             RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
             "Helvetica", FontSize -> 12, FontWeight -> "Bold", FontColor -> 
             RGBColor[0.597406, 0, 0.0527047]], 
            Cell[
             StyleData["DetailsSection", "Printout"], 
             CellMargins -> {{12, 0}, {0, 16}}, PageBreakBelow -> False, 
             FontSize -> 12]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoSection"], CellFrame -> {{0, 0}, {1, 0}}, 
             ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
             CellGroupingRules -> {"SectionGrouping", 30}, 
             CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
             RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
             "Helvetica", FontSize -> 12, FontWeight -> "Bold", FontColor -> 
             RGBColor[0.597406, 0, 0.0527047]], 
            Cell[
             StyleData["DemoSection", "Printout"], 
             CellMargins -> {{12, 0}, {0, 16}}, PageBreakBelow -> False, 
             FontSize -> 12]}, Open]], 
         Cell[
          StyleData["ManipulateSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12], 
         Cell[
          StyleData["ManipulateCaptionSection"], 
          CellFrame -> {{0, 0}, {0, 2}}, ShowCellBracket -> False, 
          CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], DefaultNewCellStyle -> 
          "ManipulateCaption", StyleMenuListing -> None, FontFamily -> 
          "Verdana", FontSize -> 12, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData["ManipulateCaption"], ShowCellBracket -> False, 
          CellMargins -> {{48, 48}, {10, 16}}, StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12, FontColor -> GrayLevel[0], 
          DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
         Cell[
          StyleData[
          "SeeAlsoSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, DefaultNewCellStyle -> "SeeAlso"], 
         Cell[
          StyleData["SeeAlso", StyleDefinitions -> StyleData["DemoNotes"]], 
          CellDingbat -> 
          Cell["\[FilledSmallSquare]", FontColor -> 
            RGBColor[0.928786, 0.43122, 0.104662]], ShowCellBracket -> False, 
          FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "RelatedLinksSection", StyleDefinitions -> 
           StyleData["DemoSection"]], ShowCellBracket -> False, 
          DefaultNewCellStyle -> "RelatedLinks"], 
         Cell[
          StyleData[
          "RelatedLinks", StyleDefinitions -> StyleData["DemoNotes"], 
           FontColor -> RGBColor[0.928786, 0.43122, 0.104662]], 
          ShowCellBracket -> False, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "CategoriesSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, DefaultNewCellStyle -> "Categories"], 
         Cell[
          StyleData["Categories", StyleDefinitions -> StyleData["DemoNotes"]],
           ShowCellBracket -> False], 
         Cell[
          StyleData[
          "AuthorSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {4, 18}}, 
          CellElementSpacings -> {"CellMinHeight" -> 3}, 
          CellFrameMargins -> {{48, 48}, {6, 3}}, DefaultNewCellStyle -> 
          "Author", FontSize -> 1, FontColor -> GrayLevel[1]], 
         Cell[
          StyleData[
          "Author", StyleDefinitions -> StyleData["DemoNotes"], FontColor -> 
           GrayLevel[0.64]], ShowCellBracket -> False], 
         Cell[
          StyleData[
          "DetailNotes", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False, FontColor -> GrayLevel[0]], 
         Cell[
          StyleData[
          "CitationSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 14}}, 
          DefaultNewCellStyle -> "Categories"], 
         Cell[
          StyleData["Citations", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False, ParagraphSpacing -> {0, 6}], 
         Cell[
          StyleData[
          "RevisionSection", StyleDefinitions -> StyleData["DemoSection"]], 
          DefaultNewCellStyle -> "RevisionNotes"], 
         Cell[
          StyleData[
          "RevisionNotes", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Specific Styles", "Section", 
          CellChangeTimes -> {{3.34971724802035*^9, 3.34971724966638*^9}, {
           3.35091840608065*^9, 3.35091840781999*^9}, {3.35091845122987*^9, 
           3.35091845356607*^9}, {3.36230868322317*^9, 3.36230868335672*^9}, {
           3.36928857618576*^9, 3.36928857640452*^9}, {3.3737586217185173`*^9,
            3.373758622077897*^9}}], 
         Cell[
          StyleData["InitializationSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12, FontColor -> 
          GrayLevel[0.45098]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["AnchorBar"], ShowCellBracket -> False, 
             CellMargins -> {{48, 44}, {3, 6}}, StyleMenuListing -> None, 
             FontFamily -> "Verdana", FontSize -> 9, FontColor -> 
             GrayLevel[0.5]], 
            Cell[
             StyleData["AnchorBar", "Presentation"], FontSize -> 18], 
            Cell[
             StyleData["AnchorBar", "SlideShow"], StyleMenuListing -> None], 
            Cell[
             StyleData["AnchorBar", "Printout"], FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["AnchorLink"], StyleMenuListing -> None, 
             ButtonStyleMenuListing -> Automatic, FontColor -> 
             RGBColor[0.5, 0.5, 0.5], 
             ButtonBoxOptions -> {
              Active -> True, ButtonFunction :> (FrontEndExecute[{
                  FrontEnd`NotebookLocate[#2]}]& ), ButtonNote -> 
               ButtonData}], 
            Cell[
             StyleData["AnchorLink", "Printout"], 
             FontVariations -> {"Underline" -> False}, FontColor -> 
             GrayLevel[0]]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["GamePadStatus"], ShowCellBracket -> False, 
             CellMargins -> {{48, 48}, {5, 5}}, StyleMenuListing -> None, 
             FontFamily -> "Verdana", FontSize -> 10], 
            Cell[
             StyleData["GamePadStatus", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoInstruction"], CellMargins -> {{48, 48}, {5, 5}}, 
             CellFrameLabelMargins -> 2, MenuSortingValue -> 800, 
             MenuCommandKey -> "8", StyleMenuListing -> None, FontFamily -> 
             "Verdana", FontSize -> 11, Background -> RGBColor[1, 0.85, 0.5], 
             DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
            Cell[
             StyleData["DemoInstruction", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, Hyphenation -> True, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, 
             LineSpacing -> {1., 2, 2.}, FontSize -> 9]}, Open]], 
         Cell[
          StyleData[
          "ImplementationSection", StyleDefinitions -> 
           StyleData["DemoSection"]], Deletable -> True, DefaultNewCellStyle -> 
          "ImplementationNotes"], 
         Cell[
          StyleData[
          "ImplementationNotes", StyleDefinitions -> StyleData["DemoNotes"]]], 
         Cell[
          StyleData[
          "StatusSection", StyleDefinitions -> StyleData["DemoSection"]], 
          DefaultNewCellStyle -> "StatusNotes"], 
         Cell[
          StyleData[
          "StatusNotes", StyleDefinitions -> StyleData["DemoNotes"]]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["SectionGloss"], StyleMenuListing -> None, FontSize -> 
             0.85 Inherited, FontWeight -> "Plain", FontColor -> 
             GrayLevel[0.6]], 
            Cell[
             StyleData["SectionGloss", "Printout"]]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineFormula"], 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, LanguageCategory -> 
             "Formula", AutoSpacing -> True, ScriptLevel -> 1, 
             AutoMultiplicationSymbol -> False, SingleLetterItalics -> False, 
             SpanMaxSize -> 1, StyleMenuListing -> None, FontFamily -> 
             "Courier", FontSize -> 1.05 Inherited, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             FractionBoxOptions -> {BaseStyle -> {SpanMaxSize -> Automatic}}, 
             GridBoxOptions -> {
              GridBoxItemSize -> {
                "Columns" -> {{Automatic}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{1.}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["InlineFormula", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineOutput"], CellHorizontalScrolling -> True, 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, LanguageCategory -> 
             None, AutoMultiplicationSymbol -> False, StyleMenuListing -> 
             None, FontFamily -> "Courier", FontSize -> 1.05 Inherited], 
            Cell[
             StyleData["InlineOutput", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineMath"], DefaultFormatType -> 
             "DefaultTextFormatType", DefaultInlineFormatType -> 
             "TraditionalForm", LanguageCategory -> "Formula", AutoSpacing -> 
             True, ScriptLevel -> 1, AutoMultiplicationSymbol -> False, 
             SingleLetterItalics -> True, SpanMaxSize -> DirectedInfinity[1], 
             StyleMenuListing -> None, FontFamily -> "Times", FontSize -> 
             1.05 Inherited, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             GridBoxOptions -> {
              GridBoxItemSize -> {
                "Columns" -> {{Automatic}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{1.}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["InlineMath", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["TableBase"], CellMargins -> {{48, 48}, {4, 4}}, 
             SpanMaxSize -> 1, StyleMenuListing -> None, FontFamily -> 
             "Courier", FontSize -> 11, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             GridBoxOptions -> {
              GridBoxAlignment -> {
                "Columns" -> {{Left}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{Baseline}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["TableBase", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}, FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "1ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.04], {
                    Scaled[0.966]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.126], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "1ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.078], {
                    Scaled[0.922]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "2ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.05], 
                   Scaled[0.41], {
                    Scaled[0.565]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.14], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "2ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.079], 
                   Scaled[0.363], {
                    Scaled[0.558]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "3ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.04], 
                   Scaled[0.266], 
                   Scaled[0.26], {
                    Scaled[0.44]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}}, 
                 "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.14], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "3ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.08], 
                   Scaled[0.25], 
                   Scaled[0.25], {
                    Scaled[0.42]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}}, 
                 "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["TableText"], Deletable -> False, StyleMenuListing -> 
             None, FontFamily -> "Verdana", FontSize -> 0.952 Inherited], 
            Cell[
             StyleData["TableText", "Printout"], 
             CellMargins -> {{24, 0}, {0, 8}}, Hyphenation -> True, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, 
             LineSpacing -> {1., 2, 2.}]}, Open]], 
         Cell[
          StyleData["Continuation"], FontColor -> GrayLevel[1]]}, Closed]]}, 
     Open]]}, Visible -> False, FrontEndVersion -> 
  "8.0 for Microsoft Windows (32-bit) (February 23, 2011)", StyleDefinitions -> 
  "Default.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{
 "ControlSuggestions"->{
  Cell[153278, 2936, 258, 7, 70, "ManipulateCaption",
   CellTags->"ControlSuggestions"],
  Cell[153539, 2945, 5495, 119, 70, "ManipulateCaption",
   CellTags->"ControlSuggestions"]},
 "Copyright"->{
  Cell[166678, 3251, 818, 23, 70, "Text",
   CellTags->"Copyright"]}
 }
*)
(*CellTagsIndex
CellTagsIndex->{
 {"ControlSuggestions", 193348, 3820},
 {"Copyright", 193548, 3825}
 }
*)
(*NotebookFileOutline
Notebook[{
Cell[1265, 31, 34328, 567, 70, "DemonstrationHeader"],
Cell[35596, 600, 37, 0, 70, "DemoTitle"],
Cell[35636, 602, 111537, 2089, 70, "Output",
 CellID->983915261],
Cell[147176, 2693, 665, 23, 70, "ManipulateCaption"],
Cell[147844, 2718, 405, 14, 70, "ManipulateCaption",
 CellID->1379222796],
Cell[148252, 2734, 245, 8, 70, "ManipulateCaption",
 CellID->659046654],
Cell[148500, 2744, 588, 22, 70, "ManipulateCaption",
 CellID->2023946199],
Cell[149091, 2768, 982, 40, 70, "ManipulateCaption",
 CellID->1719721734],
Cell[150076, 2810, 697, 27, 70, "ManipulateCaption",
 CellID->724192869],
Cell[150776, 2839, 1689, 66, 70, "ManipulateCaption",
 CellID->380778365],
Cell[152468, 2907, 517, 16, 70, "ManipulateCaption",
 CellID->1641698573],
Cell[152988, 2925, 287, 9, 70, "ManipulateCaption",
 CellID->251648375],
Cell[153278, 2936, 258, 7, 70, "ManipulateCaption",
 CellTags->"ControlSuggestions"],
Cell[153539, 2945, 5495, 119, 70, "ManipulateCaption",
 CellTags->"ControlSuggestions"],
Cell[159037, 3066, 33, 0, 70, "DetailsSection"],
Cell[159073, 3068, 53, 1, 70, "DetailNotes",
 CellID->1965504561],
Cell[159129, 3071, 232, 6, 70, "DetailNotes",
 CellID->1552175217],
Cell[159364, 3079, 44, 0, 70, "RelatedLinksSection"],
Cell[159411, 3081, 636, 21, 70, "RelatedLinks",
 CellID->670378459],
Cell[160050, 3104, 4781, 89, 70, "DemoSourceNotebookSection",
 CellGroupingRules->{"SectionGrouping", 25}],
Cell[164834, 3195, 45, 0, 70, "CitationSection"],
Cell[164882, 3197, 930, 27, 70, "Author"],
Cell[165815, 3226, 860, 23, 70, "Citations"],
Cell[166678, 3251, 818, 23, 70, "Text",
 CellTags->"Copyright"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature rRqOTCnYKyiHnxQqYi86tHhy *)
